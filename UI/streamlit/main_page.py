...
from main_page_theme import apply_theme

from dotenv import load_dotenv, find_dotenv
from openai import APIConnectionError
from models.rags.hybrid_rag import (
    retrieve,
    retrieve_hybrid,
    retrieve_semantic,
...
# Legacy imports - gpts module no longer exists, using models.agents instead
# from gpts.gpt_agent import profileAgent
# from gpts.gpt5_web import WebAgent
from models.agents.agent_assistants import general_assistant, maybe_route_to_action
from models.agents.agent_web import WebAgent  # Using agent_web instead of gpt5_web
from models.agents.agent_pdf import PDFChatModel
from models.agents.async_profile_agent import AsyncProfileAgent as profileAgent  # Import as alias for compatibility
...
from scripts.dialogues import *
from scripts.default_prompts import default_gpt_prompt,  new_system_finance_prompt, finance_prompt_web
...
async def check_actions(prompt, client, deployment) -> bool:

    calls = await maybe_route_to_action(prompt, client.az_openai, deployment)
    ...
    for call in calls:
        if call.function.name == "create_company_profile":
            ...
            out_text = client.final_text

            # Create downloadable DOCX
            docx_buffer = markdown_table_to_docx(
                out_text,
                output_path=None,  # Don't save to disk, just return buffer
            ...
            st.session_state.history.append({
                "q": prompt,
                "a": f"✅ Created a company profile for **{company}**. Download available above.\n\n---\n\n{out_text}"
            })
...
def stream_answer(prompt: str, section_build = False, section = ''):
    ...
    try:
        ...
        if section_build:
            answer_text = asyncio.run(agent._generate_section(section = section))

        else:
            if asyncio.run(check_actions(prompt,
                             agent,
                             'gpt-5'
                             )):
                pass
            else:
                resp = asyncio.run(agent._rag_answer(rag_nl = prompt, question= prompt))
...
def mix_answer(prompt):
    ...
    try:
        answer_text = asyncio.run(agent._answer(question=prompt))
    except Exception as e:
        answer_text = f"ERROR. \n {e}"
    ...
    with st.chat_message("assistant"):
        ...
        if st.session_state.websearch:
            ...
            try:
                ...
                if asyncio.run(check_actions(prompt, agent, 'gpt-5')):
